...
# Lưu vào DB
def InsertFileToDatabase(username, prompt, type, result_path, style, duration):
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO history (username, input_text, conversion_type, result, style, duration) VALUES (%s, %s, %s, %s, %s, %s)",
                (username, prompt, type, result_path, style, duration))
            connection.commit()
            return True
    except Exception as e:
...
def TextToVideo():
    ...
    try:
        ...
        InsertFileToDatabase(username, original_prompt, 'text_to_video', 'video/' + filename, style, duration)
